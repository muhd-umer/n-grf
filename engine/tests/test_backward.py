...
class TestBackward(unittest.TestCase):
    ...
    def test_rasterize_backward(self):
        ...
        N = 25
        points = torch.rand(N, 3, device=self.device, requires_grad=True)
        scaling = torch.exp(
            torch.rand(N, 3, device=self.device, requires_grad=True) - 0.5
        )
        rotation = F.normalize(
            torch.rand(N, 4, device=self.device, requires_grad=True), dim=1
        )
        attenuation = torch.rand(N, 1, device=self.device, requires_grad=True)
        phase_rotation = (
            torch.rand(N, 1, device=self.device, requires_grad=True) * 2 * math.pi
        )
        opacity = torch.sigmoid(
            torch.rand(N, 1, device=self.device, requires_grad=True)
        )
        receiver = torch.rand(3, device=self.device)
        transmitter = torch.rand(3, device=self.device)

        points_torch = points.clone().detach().requires_grad_(True)
        scaling_torch = scaling.clone().detach().requires_grad_(True)
        rotation_torch = rotation.clone().detach().requires_grad_(True)
        attenuation_torch = attenuation.clone().detach().requires_grad_(True)
        phase_rotation_torch = phase_rotation.clone().detach().requires_grad_(True)
        opacity_torch = opacity.clone().detach().requires_grad_(True)
        ...
        points_cuda = points.clone().detach().requires_grad_(True)
        scaling_cuda = scaling.clone().detach().requires_grad_(True)
        rotation_cuda = rotation.clone().detach().requires_grad_(True)
        attenuation_cuda = attenuation.clone().detach().requires_grad_(True)
        phase_rotation_cuda = phase_rotation.clone().detach().requires_grad_(True)
        opacity_cuda = opacity.clone().detach().requires_grad_(True)

        channel_cuda = cuda_rasterize(
            points_cuda,
            scaling_cuda,
            rotation_cuda,
            attenuation_cuda,
        ...
        self.assertTrue(
            torch.allclose(channel_torch, channel_cuda, rtol=1e-5, atol=1e-5)
        )

        loss_torch = channel_torch.sum()
        loss_torch.backward()

        loss_cuda = channel_cuda.sum()
        loss_cuda.backward()

        self.assertTrue(
            torch.allclose(points_torch.grad, points_cuda.grad, rtol=1e-4, atol=1e-4)
        )
        self.assertTrue(
            torch.allclose(scaling_torch.grad, scaling_cuda.grad, rtol=1e-4, atol=1e-4)
        )
        self.assertTrue(
            torch.allclose(
                rotation_torch.grad, rotation_cuda.grad, rtol=1e-4, atol=1e-4
            )
        )
        self.assertTrue(
            torch.allclose(
                attenuation_torch.grad, attenuation_cuda.grad, rtol=1e-4, atol=1e-4
            )
        )
        self.assertTrue(
            torch.allclose(
                phase_rotation_torch.grad,
                phase_rotation_cuda.grad,
                rtol=1e-4,
                atol=1e-4,
            )
        )
        self.assertTrue(
            torch.allclose(opacity_torch.grad, opacity_cuda.grad, rtol=1e-4, atol=1e-4)
        )
