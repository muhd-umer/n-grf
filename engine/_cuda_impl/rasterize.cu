...
__global__ void compute_gaussian_influence_kernel(
    const float* uv,
    const float* cov2d,
    float* influences,
    int num_tx,
    ...
    float cov[4] = {
        cov2d[idx * 4 + 0], cov2d[idx * 4 + 1],
        cov2d[idx * 4 + 2], cov2d[idx * 4 + 3]};
    ...
    for (int i = 0; i < num_tx; i++) {
        ...
        for (int j = 0; j < num_rx; j++) {
            ...
            influences[idx * num_tx * num_rx + i * num_rx + j] = expf(-0.5f * m_dist);
        }
    }
}

// CUDA kernel for computing channel contribution
__global__ void compute_channel_kernel(
...
__global__ void alpha_blending_kernel(
    ...
    for (int k = 0; k < N; k++) {
        ...
        float effective_opacity = opacity[idx] * influences[idx * num_tx * num_rx + i * num_rx + j];

        result_real += transmittance * effective_opacity * real_contributions[idx];
        result_imag += transmittance * effective_opacity * imag_contributions[idx];
...
torch::Tensor computeGaussianInfluenceCUDA(
...
torch::Tensor alphaBlendingCUDA(
    ...
    alpha_blending_kernel<<<blocks, threads>>>(
        ...
        channel_real.data_ptr<float>(),
        channel_imag.data_ptr<float>(),
        num_tx,
        num_rx,
        N);
...
std::vector<torch::Tensor> rasterizeForwardCUDA(
    ...
    const torch::Tensor& transmitter,
    int num_tx,
    int num_rx,
    float frequency) {
    ...
    auto [distances, uv, cov2d] = projectToChannelSpaceCUDA(
        points, cov3d, receiver, num_tx, num_rx);
    ...
    torch::Tensor influences = computeGaussianInfluenceCUDA(
        uv, cov2d, num_tx, num_rx);

    // Step 4: Compute channel contribution
    auto [real_contributions, imag_contributions] = computeChannelCUDA(
