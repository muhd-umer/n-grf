...
// Backward wrapper for alpha blending
...
alphaBlendingBackwardCUDA(const torch::Tensor& grad_output,
                          ...
                          const int num_rx) {
    const c10::cuda::CUDAGuard device_guard(influences.device());
    ...
    // Extract real and imaginary parts of contributions
    torch::Tensor contributions_real = torch::real(contributions);
    torch::Tensor contributions_imag = torch::imag(contributions);

    // Allocate gradients
    torch::Tensor grad_influences = torch::zeros_like(influences);
    torch::Tensor grad_contributions_real = torch::zeros({N}, options);
    torch::Tensor grad_contributions_imag = torch::zeros({N}, options);
    torch::Tensor grad_opacity = torch::zeros({N}, options);

    // Intermediate gradients for effective opacity
    torch::Tensor grad_alpha_eff = torch::zeros({N, num_tx, num_rx}, options);

    // Run alpha blending backward kernel
    {
        const int threads = num_rx;
        const int blocks = num_tx;
    ...
    // Propagate gradients from effective opacity
    {
        const int threads = 32;
        const int blocks = (N + threads - 1) / threads;
    ...
    // Combine real and imaginary gradients to complex gradient
    torch::Tensor grad_contributions =
        torch::complex(grad_contributions_real, grad_contributions_imag);

    return std::make_tuple(grad_influences, grad_contributions, grad_opacity);
}

// Backward wrapper for computing wireless channel contributions
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
computeChannelBackwardCUDA(const torch::Tensor& grad_real,
    ...
    const c10::cuda::CUDAGuard device_guard(attenuation.device());
    ...
    // Allocate gradients
    torch::Tensor grad_attenuation = torch::zeros_like(attenuation);
    torch::Tensor grad_phase_rotation = torch::zeros_like(phase_rotation);
    torch::Tensor grad_distance = torch::zeros({N}, options);

    // Run compute channel backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    compute_channel_backward_kernel<<<blocks, threads>>>(
        grad_real.data_ptr<float>(), grad_imag.data_ptr<float>(),
        attenuation.data_ptr<float>(), phase_rotation.data_ptr<float>(),
        xyz_rx_distance.data_ptr<float>(), grad_attenuation.data_ptr<float>(),
        grad_phase_rotation.data_ptr<float>(), grad_distance.data_ptr<float>(),
        wavelength, N);
...
// Backward wrapper for computing Gaussian influence
std::tuple<torch::Tensor, torch::Tensor> computeGaussianInfluenceBackwardCUDA(
    ...
    const c10::cuda::CUDAGuard device_guard(uv.device());
    const int N = uv.size(0);

    // Allocate gradients
    torch::Tensor grad_uv = torch::zeros_like(uv);
    torch::Tensor grad_cov2d = torch::zeros_like(cov2d);

    // Run compute Gaussian influence backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;
...
// Backward wrapper for projecting 3D covariance to 2D
std::tuple<torch::Tensor, torch::Tensor> projectCov3dToCov2dBackwardCUDA(
    ...
    const c10::cuda::CUDAGuard device_guard(cov3d_mat.device());
    const int N = cov3d_mat.size(0);

    // Allocate gradients
    torch::Tensor grad_cov3d = torch::zeros({N, 6}, cov3d_mat.options());
    torch::Tensor grad_jacobian = torch::zeros({N, 2, 3}, jacobian.options());

    // Run project cov3d to cov2d backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    project_cov3d_to_cov2d_backward_kernel<<<blocks, threads>>>(
        grad_output.data_ptr<float>(), cov3d_mat.data_ptr<float>(),
        jacobian.data_ptr<float>(), grad_cov3d.data_ptr<float>(),
        grad_jacobian.data_ptr<float>(), N);
...
// Backward wrapper for computing Jacobian
std::tuple<torch::Tensor, torch::Tensor> computeJacobianBackwardCUDA(
    ...
    torch::Tensor grad_r = torch::zeros({N}, r.options());

    // Run compute Jacobian backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    compute_jacobian_backward_kernel<<<blocks, threads>>>(
        grad_output.data_ptr<float>(), d.data_ptr<float>(), r.data_ptr<float>(),
        grad_d.data_ptr<float>(), grad_r.data_ptr<float>(), num_tx, num_rx, N);

    return std::make_tuple(grad_d, grad_r);
}

// Backward wrapper for mapping to channel matrix
std::tuple<torch::Tensor, torch::Tensor> mapToChannelMatrixBackwardCUDA(
    ...
    const c10::cuda::CUDAGuard device_guard(s_x.device());
    const int N = s_x.size(0);

    // Allocate gradients
    torch::Tensor grad_s_x = torch::zeros_like(s_x);
    torch::Tensor grad_s_y = torch::zeros_like(s_y);

    // Run map to channel matrix backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    map_to_channel_matrix_backward_kernel<<<blocks, threads>>>(
        grad_output.data_ptr<float>(), s_x.data_ptr<float>(),
        s_y.data_ptr<float>(), grad_s_x.data_ptr<float>(),
        grad_s_y.data_ptr<float>(), num_tx, num_rx, N);

    return std::make_tuple(grad_s_x, grad_s_y);
}

// Backward wrapper for transforming to uniform coordinates
std::tuple<torch::Tensor, torch::Tensor> transformToUniformCoordsBackwardCUDA(
    ...
    const c10::cuda::CUDAGuard device_guard(longitude.device());
    const int N = longitude.size(0);

    // Allocate gradients
    torch::Tensor grad_longitude = torch::zeros_like(longitude);
    torch::Tensor grad_latitude = torch::zeros_like(latitude);

    // Run transform to uniform coords backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;
    ...
    return std::make_tuple(grad_longitude, grad_latitude);
}

// Backward wrapper for computing spherical coordinates
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
computeSphericalCoordsBackwardCUDA(const torch::Tensor& grad_longitude,
    ...
    torch::Tensor displacement = points - receiver;
    torch::Tensor r =
        torch::sqrt(torch::sum(displacement * displacement, /*dim=*/1));

    // Allocate gradients
    torch::Tensor grad_displacement = torch::zeros_like(displacement);
    torch::Tensor grad_r = torch::zeros_like(r);

    // Run compute spherical coords backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    compute_spherical_coords_backward_kernel<<<blocks, threads>>>(
        grad_longitude.data_ptr<float>(), grad_latitude.data_ptr<float>(),
        displacement.data_ptr<float>(), r.data_ptr<float>(),
        grad_displacement.data_ptr<float>(), grad_r.data_ptr<float>(), N);

    // Gradient of points is the same as gradient of displacement
    torch::Tensor grad_points = grad_displacement;

    return std::make_tuple(grad_points, grad_displacement, grad_r);
}

// Backward wrapper for computing distances to receiver
torch::Tensor computeDistancesToReceiverBackwardCUDA(
    ...
    // Run compute distances to receiver backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    compute_distances_to_receiver_backward_kernel<<<blocks, threads>>>(
        grad_output.data_ptr<float>(), points.data_ptr<float>(),
        receiver.data_ptr<float>(), grad_points.data_ptr<float>(), N);

    return grad_points;
}

// Backward wrapper for computing 3D covariance from scaling and rotation
std::tuple<torch::Tensor, torch::Tensor>
computeCov3dFromScalingRotationBackwardCUDA(const torch::Tensor& grad_cov3d,
                                            const torch::Tensor& scaling,
                                            const torch::Tensor& rotation,
                                            const float scale_modifier) {
    const c10::cuda::CUDAGuard device_guard(scaling.device());
    const int N = scaling.size(0);

    // Allocate gradients
    torch::Tensor grad_scaling = torch::zeros_like(scaling);
    torch::Tensor grad_rotation = torch::zeros_like(rotation);

    // Run compute 3D covariance from scaling and rotation backward kernel
    const int threads = 32;
    const int blocks = (N + threads - 1) / threads;

    compute_cov3d_from_scaling_rotation_backward_kernel<<<blocks, threads>>>(
        grad_cov3d.data_ptr<float>(), scaling.data_ptr<float>(),
        rotation.data_ptr<float>(), grad_scaling.data_ptr<float>(),
        grad_rotation.data_ptr<float>(), scale_modifier, N);
